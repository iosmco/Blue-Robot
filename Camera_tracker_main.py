...
class App(QWidget):
    ...
    def __init__(self):
        ...
        self.cap.set(3, 640)                        #set capture width
        self.cap.set(4, 460)                        #set capture height
    ...
    def initUI(self):     #UI related stuff
        self.setWindowTitle('BLUE ROBOT')              #set window title
    ...
    def connect(self):    #set COM port from text box if arduino not already connected
        if(not self.is_connected):
            ...
            if (self.ard.connect(port)):    #set port label message
                self.COMConnectLabel.setText("Arduino Successfully Connected to Port ["+port+"]")
            else:
                self.COMConnectLabel.setText("Failed to Connect the  Arduino to Port ["+port+"]")
    ...
    def update_angles(self):  #update variables from text boxes
        try:
            ...
            self.ui.label_info.setText("values updated")
        except:
            print("can't update values")
            self.ui.label_info.setText("can't update values")
        ...
        while(self.rec):
            ...
            if(self.is_connected):            #if arduino connected
                if(self.manual_mode):         #and manual mode on
                    ...
                    if self.ui.radioBtnFaceMode.isChecked():
                        processed_img = self.image_process(img) #process image (check for faces and draw circle and cross)
                        faceMode = True
                        colorMode = False

                    if self.ui.radioBtnColorMode.isChecked():
                        processed_img = self.color_process(img)
                        faceMode = False
                        colorMode = True
    ...
    def update_GUI(self, openCV_img): # Convert openCV image and update pyQt label
        ...
        try: #if this doesn't work check camera ID
            openCV_img = cv2.resize(openCV_img, (640, 460))  #this is stretching the image a bit but if not done it won't fit the UI
    ...
    def color_process(self, img):
        processed_img = opr.find_color(img, self.max_target_distance)
        if(processed_img[0]):
            self.face_detected = True
            self.empty_frame_number = self.max_empty_frame
            self.target_locked = processed_img[4]
            self.move_color_mode(processed_img[2], processed_img[3])
            return processed_img[1]
        else:
            self.face_detected = False
            self.target_locked = False
